...
export default function AdminForms({assets,canEdit}:{assets:Asset[];canEdit:boolean}){
  const [status,setStatus]=useState(''); const [lookup,setLookup]=useState<Lookup>({}); const [mapUrl,setMapUrl]=useState(''); const [looking,setLooking]=useState(false);
  async function submit(url:string,body:BodyInit){setStatus('Saving…');const response=await fetch(url,{method:'POST',body});const result=await response.json();setStatus(response.ok?(url==='/api/documents'?'Manual saved. Refreshing…':'Saved.'):result.error||'Unable to save.');if(response.ok&&url==='/api/documents')setTimeout(()=>window.location.reload(),600)}
  async function resolveMap(){if(!mapUrl)return;setLooking(true);setStatus('Reading the Google Maps link…');const response=await fetch('/api/places/resolve',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({url:mapUrl})});const result=await response.json();setLookup(result);setStatus(response.ok?'Place found. Check the details, then save it.':result.error||'Unable to read that link.');setLooking(false)}
  ...
  return <><div className="admin-tabs">
    <article className="admin-panel"><h2>Upload a manual</h2><p>PDF, text, or Markdown; up to 50 MB.</p><form onSubmit={(event:FormEvent<HTMLFormElement>)=>{event.preventDefault();submit('/api/documents',new FormData(event.currentTarget))}}>
      ...
      <label><input name="is_published" type="checkbox" defaultChecked/> Publish for visitors</label><button>Upload manual</button>
    </form></article>
    <article className="admin-panel"><h2>Save a place</h2><p>Paste a Google Maps sharing link. No paid Google API is used.</p>
      ...
      <form onSubmit={(event:FormEvent<HTMLFormElement>)=>{event.preventDefault();submit('/api/places',new FormData(event.currentTarget))}}><input type="hidden" name="google_maps_url" value={lookup.url||mapUrl}/>
        ...
        <label>Our photograph (optional)<input name="image" type="file" accept="image/jpeg,image/png,image/webp,image/heic"/></label><label>Notes<textarea name="notes" rows={4}/></label><button>Save place</button>
      </form>
    </article>
